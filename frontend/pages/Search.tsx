  ...
  const filteredFacilities = useMemo(() => {
    return facilities
      .filter(f => {
        ...
        const matchesStates = selectedStates.length === 0 || selectedStates.includes(f.state);
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 sticky top-24 max-h-[calc(100vh-120px)] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              {(selectedCategories.length > 0 || selectedTags.length > 0 || selectedStates.length > 0) && (
                <button 
                  onClick={() => { setSelectedCategories([]); setSelectedTags([]); setSelectedStates([]); }}
            ...
            <div className="mb-6">
              <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Categories</label>
                ...
                {CATEGORIES.map(cat => (
                  <label key={cat} className="flex items-center group cursor-pointer">
                    <input 
                      ...
                      checked={selectedCategories.includes(cat as FacilityCategory)}
                      onChange={() => toggleCategory(cat)}
                    />
                    <span className="ml-3 text-sm text-slate-600 group-hover:text-indigo-600 transition-colors">{cat}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="mb-6">
              <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">States</label>
            ...
            <div className="mb-6">
              <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Popular Tags</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Sort By</label>
              <select 
                value={sortBy}
        ...
        <div className="flex-1">
          <div className="mb-8 relative">
            <input 
              ...
              className="w-full pl-12 pr-4 py-4 rounded-xl bg-white border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 shadow-sm"
            />
          ...
          {filteredFacilities.length > 0 ? (
            ...
            <div className="bg-white rounded-2xl p-12 text-center border border-dashed border-slate-300">
              <div className="bg-slate-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.172 9.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
              </div>
